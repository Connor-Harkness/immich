...
const ValidateLibraryImportPathResponseSchema = z
  .object({
    ...
    isValid: z.boolean().default(false).describe('Is valid'),
...
const LibraryStatsResponseSchema = z
  .object({
    photos: z.int().default(0).describe('Number of photos'),
    videos: z.int().default(0).describe('Number of videos'),
    total: z.int().default(0).describe('Total number of assets'),
    usage: z.int().default(0).describe('Storage usage in bytes'),
  })
