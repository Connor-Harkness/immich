...
const AlbumsResponseSchema = z
  .object({
    defaultAssetOrder: AssetOrderSchema.default(AssetOrderSchema.enum.Desc),
  })
  .meta({ id: 'AlbumsResponse' });

const FoldersResponseSchema = z
  .object({
    enabled: z.boolean().default(false).describe('Whether folders are enabled'),
    sidebarWeb: z.boolean().default(false).describe('Whether folders appear in web sidebar'),
  })
  .meta({ id: 'FoldersResponse' });

const MemoriesResponseSchema = z
  .object({
    enabled: z.boolean().default(true).describe('Whether memories are enabled'),
    duration: z.int().default(5).describe('Memory duration in seconds'),
  })
  .meta({ id: 'MemoriesResponse' });

const PeopleResponseSchema = z
  .object({
    enabled: z.boolean().default(true).describe('Whether people are enabled'),
    sidebarWeb: z.boolean().default(false).describe('Whether people appear in web sidebar'),
  })
  .meta({ id: 'PeopleResponse' });

const RatingsResponseSchema = z
  .object({
    enabled: z.boolean().default(false).describe('Whether ratings are enabled'),
  })
  .meta({ id: 'RatingsResponse' });

const SharedLinksResponseSchema = z
  .object({
    enabled: z.boolean().default(true).describe('Whether shared links are enabled'),
    sidebarWeb: z.boolean().default(false).describe('Whether shared links appear in web sidebar'),
  })
  .meta({ id: 'SharedLinksResponse' });

const TagsResponseSchema = z
  .object({
    enabled: z.boolean().default(true).describe('Whether tags are enabled'),
    sidebarWeb: z.boolean().default(true).describe('Whether tags appear in web sidebar'),
  })
...
const DownloadResponseSchema = z
  .object({
    ...
    includeEmbeddedVideos: z.boolean().default(false).describe('Whether to include embedded videos in downloads'),
  })
...
const CastResponseSchema = z
  .object({
    gCastEnabled: z.boolean().default(false).describe('Whether Google Cast is enabled'),
  })
